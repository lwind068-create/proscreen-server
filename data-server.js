...
app.get('/api/debug/price/:ticker', async (req, res) => {
  ...
  try {
    ...
    results.fmp_profile = await r.json();
  } catch(e) { results.fmp_profile = { error: e.message }; }
  try {
    const r = await fetch(`https://financialmodelingprep.com/stable/ratios-ttm?symbol=${ticker}&apikey=${FMP_KEY}`);
    results.fmp_ratios = await r.json();
  } catch(e) { results.fmp_ratios = { error: e.message }; }
  res.json(results);
});
...
async function fetchPrice(ticker) {
  ...
  try {
    ...
    if (res.ok) {
      ...
      if (q && q.price > 0) {
        let changePct = q.changesPercentage || 0;
        if (!changePct && q.previousClose && q.previousClose > 0) changePct = ((q.price - q.previousClose) / q.previousClose) * 100;
        if (!changePct && q.change && q.previousClose) changePct = (q.change / q.previousClose) * 100;
        const result = {
