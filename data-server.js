...
async function fetchMergedStock(ticker) { const [pr, fr] = await Promise.allSettled([fetchPrice(ticker), fetchFundamentals(ticker)]); const p = pr.status === 'fulfilled' ? pr.value : null; const f = fr.status === 'fulfilled' ? fr.value : {}; if (!p) return null; return { ticker, name: f.name || ticker, sector: normalizeSector(f.sector || ''), mcapVal: f.mcapVal || 0, price: p.price, change: p.change, volumeM: p.volumeM, open: p.open, high: p.high, low: p.low, pe: f.pe != null ? f.pe : -1, divYield: f.divYield || 0, eps: f.eps || 0, roe: f.roe || 0, debtEq: f.debtEq || 0, revGrowth: f.revGrowth || 0, beta: f.beta || 1, week52High: f.week52High || p.price * 1.1, week52Low: f.week52Low || p.price * 0.9, momentum: calcMomentum(p.change, f.revGrowth || 0, f.roe || 0) }; }
